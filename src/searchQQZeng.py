...
class IpSearch:   
    ...
    def __init__(self,file_name):       
        try:
            datfile = os.path.join(os.path.dirname(__file__), file_name)            
            f= open(datfile, 'rb') 
            if mmap is not None:
    ...
    def int_from_3byte(self,offset):
        return struct.unpack('<L', self.data[offset:offset + 3]+b'\0' )[0]

    def int_from_1byte(self,offset):
        return struct.unpack('B', self.data[offset:offset + 1])[0]

def searchFromQQZeng(data):
    ipsearch = IpSearch("qqzeng-ip-utf8.dat")
    ipstr = ""
    for i in data:
        try:
            ip = i.split('/')[0]
            if ip:
                ...
                ipinfo = f"{ip} {iplist[1]} {iplist[2]} {iplist[3]} {iplist[4]} {iplist[5]} {iplist[8]}".strip()
...
if __name__ == "__main__":
    ...
    # ip = "171.212.114.217"
    ip = "1.1.1.1"
    # ipres = ipsearch.Find("171.212.114.217").replace("|", " ").strip()
    iplist = ipsearch.Find(ip).split("|")
    ipstr = f"{ip} {iplist[1]} {iplist[2]} {iplist[3]} {iplist[4]} {iplist[5]} {iplist[8]}".strip() + "\n"
    ipstr = re.sub(r"\s+", " ", ipstr)
    # ipinfo = re.sub(r"\s+", " ", ipres)
    print(ipstr)